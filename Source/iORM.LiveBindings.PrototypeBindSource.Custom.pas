...
type
  ...
  private
    ...
    // Quest� � una collezione dove eventuali BindSource di dettaglio
...
begin
  ...
  // Quest� � una collezione dove eventuali BindSources di dettaglio
  // si registrano per rendere nota la loro esistenza al Master. Sar� poi
  // usata dal Master per fare in modo che, quando viene richiesta la creazione
  // del suo BindSourceAdapter (del master), venga scatenata anche la creazione
  // anche di tutti gli adapters relativi ai presenters di dettaglio (che si sono
...
begin
  // Qui forzo l'AutoPost a True perch� ridichiarare la propriet� con default = True
  //  non � stato sufficiente anche perch� il getter e setter sono privati e statici nell'antenato.
  AutoPost := True;
...
// ----- OLD CODE -----
  ...
  // Qui siamo subito dopo il caricamento dei valori delle propriet� dal file DFM
  //  e se la propriet� Preview = True scatena il relativo metodo set per far si
  //  che venga posta a true anche la propriet� AutoActivate e rendere visibile
  //  i dati a desig-time
  SetPreview(Preview);
...
//  // NB: Ho dovuto attivare automaticamnete il BindSource (nel caso non lo fosse gi�) perch�
//  //  altrimenti avevo degli AV dovuti al fatto che il BSA non esisteva
//  // NB: Se il BS non � mai stato attivo prima allora crea anche il relativo ActiveBindSourceAdapter
//  else
//  begin
//    if Assigned(ADataObject) then
//    begin
...
begin
  ...
  //  la propriet� "AutoActivate" per mostrare i dati anche a design-time
  FPreview := Value;
